...
def analysis(classifier_object,x,y,X_train,y_train,X_test,y_test):
    scores = cross_val_score(classifier_object,x,y,cv=8)
    print("Cross-Validation Scores :",scores)
    print("Cross-Validation Accuracy :",scores.mean() , " Deviation(+/-) :",scores.std())
    svc_pred = classifier_object.predict(X_test)
    print("Accuracy of fit:",accuracy_score(y_test,svc_pred))
...
def analysis_of_model(gt_frame,out_frame,s):
    correct = 0
    total=0
    
    for i,j in zip(gt_frame[gt_frame.columns[1]],out_frame[out_frame.columns[1]]):
        if i==j:
            correct=correct+1
        total=total+1
    print("Analysis of Model ",s)
    print("Total = ",total)
    print("Correct =",correct)
    ratio  = correct/total 
    print("Accuracy (correctly classified samples / total no. of samples) = ",ratio,"\n")
    return ratio

def main():
    if len(sys.argv)< 3:
        print("Enter Proper Arguments")
        print(" *** test.py path/test.csv path/gt.csv ")
        sys.exit(1)
    ...
    #Reading test.csv in data
    
    #Preprocessing the data
    features_encoded = pre_process(data)
    
    out_frame1 = pd.DataFrame(columns=['Id','SaleStatus'])
    out_frame1['Id']=features_encoded['Id'].astype(np.int64)
    out_frame2 = pd.DataFrame(columns=['Id','SaleStatus'])
    out_frame2['Id']=features_encoded['Id'].astype(np.int64)
    
    model_file1="final_model1.pkl"
    model_file2="final_model2.pkl"
    #Reading the model from pkl file
    classifier1 = joblib.load(model_file1)
    classifier2 = joblib.load(model_file2)
    
    #Normalization of Data
    a=list(features_encoded.columns)
    a.remove('Id')
    features = features_encoded[a]
    ...
    #Predicting SaleStatus on Test data
    svc_pred1 = classifier1.predict(features_normalized)
    svc_pred2 = classifier2.predict(features_normalized)
    
    out_frame1['SaleStatus']=pd.Series(svc_pred1)
    out_frame2['SaleStatus']=pd.Series(svc_pred2)
    ...
    gt_frame = pd.read_csv(gt_file)
    
    if(gt_frame.shape!=out_frame1.shape):
        print("\nWrong gt file, terminating the script\n")
        sys.exit(1)
    
    if(gt_frame.shape!=out_frame2.shape):
        print("\nWrong gt file, terminating the script\n")
        sys.exit(1)



    ratio1 = analysis_of_model(gt_frame,out_frame1,"1")
    ratio2 = analysis_of_model(gt_frame,out_frame2,"2")

    if(ratio1>ratio2):
         #Saving the output on out.csv  
        out_frame1.to_csv('out.csv')
    else:
         #Saving the output on out.csv  
        out_frame2.to_csv('out.csv')
